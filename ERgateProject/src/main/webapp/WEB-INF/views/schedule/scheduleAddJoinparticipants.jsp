...
<body>
	...
	<div class="popup-outer" >
		...
		<div class="popup-content">
			...
			<button type="button" class="bigBtn" id="joinSubmit" style="margin-left: 680px;">등록</button>
			...
			<script>
				$(document).on("click","#joinSubmit",function(){				
					...
					var v = $(".joinView tbody").text();
					
						if(v !=""){
						
						// 수신참조자이름 출력/변수에 담기
						var joinName=new Array();
						cnt = $("#joinList tbody input[name=empName]").length;
						for(var i=0;i<cnt;i++){
							
							joinName.push($("#joinList tbody input[name=empName]").eq(i).val());
							opener.document.getElementById("joinArea").innerHTML+= 
										$("#joinList tbody input[name=empName]").eq(i).val()+" ";
						}
						
						
						// 수신참조자 아이디 변수에 담기
						var joinId=new Array();
						cnt = $("#joinList tbody input[name=empId]").length;
						for(var i=0;i<cnt;i++){
							joinId.push($("#joinList tbody input[name=empId]").eq(i).val());
						}
						
						
						opener.getJoinId(joinId);
						opener.getJoinName(joinName);
					...
					<div id="joinArea">
						...
						<div id="signSelDiv">
							<div style="width:100%;height:645px;overflow:auto;">
								<table id="joinList" class="boardTable joinView">
								<thead>
									<tr>
										<th width="100">이름</th>
										<th>부서</th>
	...
	$(document).on("click",".selRef",function(){
		$('.checkBox:checked').each(function() {
			...
			if(regExp.test($(".joinView tbody").html())){
				console.log("중복");
				return false;
			}
